...
private:
    ...
    int readData()
    {
        ...
        fd = static_cast<int>(socket_handle);
#else
        fd = socket(AF_INET, SOCK_DGRAM, 0);
#endif
...
#ifdef _WIN32
        InetPtonA(AF_INET, ars548_multicast_ip.c_str(), &mreq.imr_multiaddr);
        InetPtonA(AF_INET, ars548_local_ip.c_str(), &mreq.imr_interface);
#else
        inet_pton(AF_INET, ars548_multicast_ip.c_str(), &mreq.imr_multiaddr);
        inet_pton(AF_INET, ars548_local_ip.c_str(), &mreq.imr_interface);
#endif
