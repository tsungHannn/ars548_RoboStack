...
class TrafficMonitor(Node):
    ...
    def setup_parameters(self, config_path):
        ...
        if self.config.model_enabled:
            if self.config.model.modality == 'camera': # 相機物件偵測(YOLO)
                ...
                self.get_logger().info(f'Model modality: {self.config.model.modality}, Model loaded on device {self.model.device}')
                ...
                self.get_logger().info(f'Model modality: {self.config.model.modality}, Model loaded on device {self.device}')
        ...
        if self.config.optimize:
            ...
            self.visualize_pub = self.create_publisher(Image, '/optimize_visualize', 1)
    ...
    def setup_topics(self):
        ...
        self.pub_track = self.create_publisher(Image, self.config.track_topic, 2)
        self.pub_radar_project = self.create_publisher(Image, self.config.radar_project_topic, 2)
        self.pub_final = self.create_publisher(Image, self.config.final_topic, 2)
        ...
        self.pub_radar_original = self.create_publisher(Image, "/radar_original_project", 1)
        self.pub_range = self.create_publisher(MarkerArray, "/radar_range", 1)
        self.pub_radar_filter = self.create_publisher(PointCloud2, '/radar_filter', 1)
        self.pub_radar_xyz_vxvy_id = self.create_publisher(PointCloud2, '/radar_object_xyz_vxvy_id', 1)
        self.pub_radar_marker = self.create_publisher(MarkerArray, '/radar_marker', 1)
        ...
        self.sub_matrix = self.create_subscription(Float32MultiArray, '/control/matrix', self.matrix_callback, 1)
        self.sub_optimize_action = self.create_subscription(String, '/control/optimize_action', self.control_callback, 1)
    ...
    def setup_traffic_stats(self):
        ...
        self.stats_interval = Duration(seconds=self.config.stats_interval_seconds or 15.0)
    ...
    def reset_traffic_stats(self, current_time=None):
        ...
        if current_time is None:
            ...
            self.stats_start_time = self.get_clock().now()
        else:
    ...
    def drawRadarRange(self):
        ...
        for t in [[0, 0, 0]]:
            radar_transform = t
            range_marker = Marker(
                header=Header(frame_id="ARS_548", stamp=self.get_clock().now().to_msg()),
                id=id,
    ...
    def vis_traffic_result(self, frame, objects):
        ...
        elapsed_sec = (self.get_clock().now().nanoseconds - self.stats_start_time.nanoseconds) * 1e-9
        cv2.putText(frame, f'Last Update: {math.ceil(elapsed_sec)} sec', (20, original_height + 120),
    ...
    def vis_recorded_data(self):
        ...
        if len(self.saved_filter_optimize_data) == 0:
            ...
            self.get_logger().info("目前沒有已儲存的資料可視化")
            return
    ...
    def process_sensor_data(self, timer):
        ...
        #     return
        if not hasattr(self, '_log_sensor_data_once'):
            self.get_logger().info('Processing sensor data')
            self._log_sensor_data_once = True
        ...
        if self.optimize == True:
            ...
            self.get_logger().info(f"最佳化完成，共優化: {len(self.saved_filter_optimize_data)} 幀，最小損失: {result.fun}")
            
            # rospy.loginfo("最佳參數: %s", str(result.x))
            self.get_logger().info(f"最佳參數: {result.x}")
            ...
            self.get_logger().info(f"轉換回角度: {result_euler_param}")
            self.get_logger().info(f"原始參數: {self.extrinsic_param_quat}")
            self.get_logger().info(f'Processing time: {(end_time - start_time).nanoseconds * 1e-9:.3f}s')
