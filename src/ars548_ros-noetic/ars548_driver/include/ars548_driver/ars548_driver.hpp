...
class Ars548_Driver {    
    ...
    void fillCloudMessage(sensor_msgs::PointCloud2 &cloud_msg){
        ...
        // cloud_msg.header.stamp.nsec=std::chrono::duration_cast<std::chrono::nanoseconds>(duration).count();
        ...
        // cloud_msg.header.stamp.nsec = now.nsec;
        // cloud_msg.header.stamp.sec = now.sec;
    ...
    void fillDirectionMessage(geometry_msgs::PoseArray &cloud_Direction,Object_List object_List,u_int32_t i){
        ...
        // cloud_Direction.header.stamp.nsec=std::chrono::nanoseconds().count();
        ...
        // cloud_Direction.header.stamp.nsec = now.nsec;
        // cloud_Direction.header.stamp.sec = now.sec;
        cloud_Direction.poses[i].position.x = double(object_List.ObjectList_Objects[i].u_Position_X);
        cloud_Direction.poses[i].position.y = double(object_List.ObjectList_Objects[i].u_Position_Y);
        cloud_Direction.poses[i].position.z = double(object_List.ObjectList_Objects[i].u_Position_Z);
        yaw = atan2(object_List.ObjectList_Objects[i].f_Dynamics_RelVel_Y, object_List.ObjectList_Objects[i].f_Dynamics_RelVel_X);   
	...
	// }
        ...
        if (
	    setsockopt(
                fd, IPPROTO_IP, IP_MULTICAST_IF, (char*) &mreq.imr_interface, sizeof(mreq.imr_interface)
	    ) < 0
        ){
            perror("setsockopt");
            return 1;
	    ...
	    //std::cout << nbytes << std::endl;
            ...
            case STATUS_MESSAGE_PAYLOAD:
                ...
                if(status.MethodID==STATUS_MESSAGE_METHOD_ID && status.PayloadLength==STATUS_MESSAGE_PDU_LENGTH){
                    ...
                    pubs["Status"].publish(statusMessage);
                }
                break;
            case OBJECT_MESSAGE_PAYLOAD:
                ...
                if(object_List.MethodID==OBJECT_MESSAGE_METHOD_ID && object_List.PayloadLength==OBJECT_MESSAGE_PDU_LENGTH){
                     //Changes all of the > 8bit data to little endian
                        ...
                        pubs["PointCloudObject"].publish(cloud_msgObj);
                        pubs["DirectionVelocity"].publish(cloud_Direction);
                        pubs["ObjectList"].publish(objectMessage);
            ...
            case DETECTION_MESSAGE_PAYLOAD:
                ...
                if(detectionList.MethodID==DETECTION_MESSAGE_METHOD_ID && detectionList.PayloadLength==DETECTION_MESSAGE_PDU_LENGTH){
                    ...
                    fillDetectionMessage(detectionMessage,detectionList);
                        ...
                        pubs["PointCloudDetection"].publish(cloud_msgDetect);
                        pubs["DetectionList"].publish(detectionMessage);
